...
func (s *SSMRemoteAccessStrategy) performSSMRegistration(ctx context.Context, pod *corev1.Pod, workspace *workspacesv1alpha1.Workspace, accessStrategy *workspacesv1alpha1.WorkspaceAccessStrategy) error {
	...
	// Step 2: Run register-ssm.sh with environment variables passed via stdin
	logger.Info("Running SSM registration script in sidecar")
	region := s.ssmClient.GetRegion()
	
	// Use stdin to pass sensitive values securely
	cmd := []string{"bash", "-c", "read ACTIVATION_ID && read ACTIVATION_CODE && read REGION && env ACTIVATION_ID=\"$ACTIVATION_ID\" ACTIVATION_CODE=\"$ACTIVATION_CODE\" REGION=\"$REGION\" " + SSMRegistrationScript}
	stdinData := fmt.Sprintf("%s\n%s\n%s\n", activationId, activationCode, region)
